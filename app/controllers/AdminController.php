...
class AdminController {
    ...
    public function eliminarUsuario() {
        ...
        if (!$id_usuario) {
            ...
            if ($id_usuario == 1) {
                $mensaje = "No puedes eliminar el usuario administrador principal.";
            } else {
                // Ejecutar el borrado
                ...
                try {
                    ...
                    $_SESSION['mensaje_usuario_error'] = "❌ Error al eliminar usuario";
                }
            }
        }
    ...
    public function actualizarUsuario() {
        ...
        $stmt = $pdo->prepare("UPDATE usuarios SET usuario=?, nombre=?, apellido1=?, apellido2=?, telefono=?, email=? WHERE id_usuario=?");
        try {
            $stmt->execute([$usuario, $nombre, $apellido1, $apellido2, $telefono, $email, $id_usuario]);
    ...
    public function descargarPresupuesto() {
        ...
        // Conversión formato de fecha a 'd-m-Y':
        ...
        $pdf->SetFont('DejaVu', '', 10);
        $pdf->Cell(0, 6, 'Cliente: ' . $cliente, 0, 1, 'L');
        $pdf->Cell(0, 6, 'Dirección: ' . $direccion_cliente, 0, 1, 'L');
        $pdf->Ln(8);

        // Tabla CABECERA (Imagen, Concepto, Cantidad, Precio, Total)
        $w = [28, 52, 18, 28, 28]; // Anchos columnas: Imagen, Concepto, Cantidad, Precio, Total
        ...
        for ($i = 0; $i < count($conceptos); $i++) {
            ...
            // Calcular líneas reales vía método de la clase MyPDF (NO como función global)
            $nblines = $pdf->NbLines($w[1], $concepto);

            $highConcepto = $nblines * $lineHeight;
            $h            = max($altoMinFila, $highConcepto);

            // --- IMAGEN, centrada y alineada ---
            $anchoImg = min(25, $w[0]); // Recomiendo 25mm para que no sobresalga
            $altoImg  = 25;
            $pdf->Cell($w[0], $h, '', 1, 0, 'C', true);
            if ($imagenes && isset($imagenes['tmp_name'][$i]) && $imagenes['tmp_name'][$i] !== '' && $imagenes['error'][$i] == 0) {
                ...
                if ($ext) {
                    ...
                    $xImg = $x + ($w[0] - $anchoImg) / 2;
                    $yImg = $y + ($h - $altoImg) / 2;
                    $pdf->Image($tmp_img, $xImg, $yImg, $anchoImg, $altoImg);

                    unlink($tmp_img);
                }
            }

            // --- CONCEPTO ---, rellena si hace falta para igualar altura
            $xConcepto = $x + $w[0];
            $yConcepto = $y;
            $pdf->SetXY($xConcepto, $yConcepto);
            $pdf->MultiCell($w[1], $lineHeight, $concepto, 1, 'C', true);

            $usedH = $nblines * $lineHeight;
            if ($usedH < $h) {
                $pdf->SetXY($xConcepto, $yConcepto + $usedH);
                $pdf->Cell($w[1], $h - $usedH, '', 'LR', 0, '', false);
            }

            // --- RESTO DE COLUMNAS ---
            $pdf->SetXY($xConcepto + $w[1], $y);
            $pdf->Cell($w[2], $h, $cantidad, 1, 0, 'C', true);
            $pdf->Cell($w[3], $h, $precio_unitario . ' €', 1, 0, 'R', true);
            $pdf->Cell($w[4], $h, $total_item . ' €', 1, 0, 'R', true);

            $pdf->SetY($y + $h); // A la siguiente fila
        }

        $pdf->Ln(8);

        // Total y Total con IVA debajo
